...
Agent $i$ has start state $(x_i^s, y_i^s, h_i^s)$ and goal $(x_i^g, y_i^g, h_i^g)$, where the required parking heading is inferred from the adjacent separator wall when omitted. The goal is satisfied only when the agent arrives at the correct pose (facing the divider) while making sure that no other agents go onto the cell after the agent has occupied its designated parking spot.

Two paths conflict via a **vertex conflict** — agents $i, j$ both at $(x, y, t)$ — or an **edge conflict** — agents swapping cells between $t$ and $t+1$. The goal is a joint plan $\Pi = \{\pi_1, \ldots, \pi_N\}$ minimizing makespan $\max_i |\pi_i|$ subject to collision freedom, with CBS additionally minimizing sum-of-costs $\sum_i |\pi_i|$.

### 2.3 Assumptions

The environment is fully observable and static meaining no dynamic obstacles beyond agents. Motion is synchronous so actions execute instantaneously and deterministically. Each cell holds at most one agent and is large enough for one vehicle.
...
Both CBS and PP use **space-time A\*** over $(x, y, h, t)$. For each agent we precompute a BFS distance map $d(x,y)$ from the goal, ignoring heading and time. This heuristic is admissible because no action reduces grid distance by more than 1 per timestep. Also, the states are packed into a 64-bit key for $O(1)$ lookup:
...
For our implementation of CBS we have a min-heap of **constraint-tree (CT) nodes**, each holding a constraint set $C$ and one cost-optimal path per agent satisfying $C$. For each expansion, the joint solution is checked for a conflict. If theres none, the solution is optimal and returned. Otherwise, a conflict between agents $i$ and $j$ spawns two children where one is a vertex or edge constraint on $i$, the other on $j$ and only the constrained agent replans via our low-level A\*. CBS is complete and optimal when the low-level planner is optimal. We set a hard cap of 10,000 CT nodes and return the best plan found.

### 3.3 Prioritized Planning (PP)

For out PP implementation the agents are planned sequentially in priority order $\sigma$. The priority order is based on start $y$ first, favoring interior agents, because the parking spots are lower on the grid and the agents spawn at the top (so agents with lower y are closer to parking spots). Each agent runs space-time A\* against a growing reservation set $R \subset \mathbb{Z}^2 \times \mathbb{Z}_{\ge 0}$ of space-time points occupied by already-planned agents. Swap conflicts are blocked inline: a move from $(x_1,y_1) \to (x_2,y_2)$ at time $t$ is rejected if $(x_2,y_2,t-1) \in R$. Goal acceptance additionally requires no future entry $(x^g, y^g, t') \in R$ for $t' > t_\text{cur}$. If any agent fails Pass 1, their start cells are permanently added to $R$ and all remaining agents are replanned (Pass 2 recovery), breaking deadlocks caused by stuck agents blocking interior paths.
...
| Two Lots Gauntlet | 30 × 26 | 16 per side | Compact 2-cell bottleneck variant |

The **gauntlet** maps are the primary benchmark: top agents are assigned bottom spots and bottom agents top spots, so all must cross a central 2-cell corridor in opposing directions — a maximally adversarial coordination test. We sweep $N \in \{4, 8, 12, 16, 20, 24\}$ with both algorithms. We also wrote a validating script that verifies the plan that was made does not break any rules.
...
CBS is strictly optimal for $N \le 8$ (sub-10 ms). At $N = 12$ it hits the node cap (487 ms) and its runtime plateaus near 610 ms for all larger counts and the cap cost dominates. PP scales near-linearly in $N$, staying under 12 ms through 24 agents and running exactly $N$ low-level A\* searches with no CT expansion. The root CT node already contains 2–52 conflicts at the gauntlet (growing with $N$), confirming the head-on structure is adversarial to CBS's branching. For $N \le 8$, PP's makespan overhead is small (5–10%); beyond the CBS tractability threshold the gap widens to 33–46%, as PP's greedy priority ordering forces agents to wait serially rather than interleaving. At $N = 24$ PP's two-pass recovery skips one agent, the first observed failure. 

---

## 5. Limitations

**CBS scalability.** CBS is exponential in the number of conflicts and becomes unusable beyond ~8–10 agents especially in dense scenarios like our gauntlet maps. The node cap bounds runtime but provides no guarantee on solution quality; the gap between capped and optimal solutions is unbounded in general.

**PP incompleteness.** PP is incomplete: a fixed priority ordering can deadlock even when a valid joint plan exists. The two pass recovery approach mitigates this issue but does not eliminate it for example one agent is skipped at $N = 24$. Dynamic re-prioritization could recover more failures but is not implemented.
...
**Open-loop execution.** Lastly, both algorithms produce offline plans with no replanning capability. Noise, unexpected obstacles, or agent deviations invalidate the plan without any recovery mechanism. Thus, actual real world deployment would require an online reactive planning approach. 

---

## 6. Conclusion

We presented a dual-algorithm MAPF system for autonomous vehicle parking with motion primatives. The 4D state space with explicit heading, BFS-precomputed admissible heuristic, 2-timestep non-in-place turns, and two-pass PP recovery combine to handle real parking-lot geometry for MAPF formulations.

CBS and PP define a clear optimality-scalability tradeoff: CBS is optimal for small groups (up to 8 agents) while PP provides near-real-time planning for 20+ agents at moderate quality cost. The gauntlet benchmark isolates the coordination challenge and reveals a phase transition at 12 agents where CBS's exponential blowup becomes dominant.

Future directions include anytime CBS / ECBS for bounded-suboptimal solutions, dynamic priority reordering to eliminate PP deadlocks, continuous-time extensions for vehicle speeds, and online replanning for closed-loop execution.
